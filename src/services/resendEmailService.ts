...
const validateEmailConfig = (): boolean => {
  ...
  if (!apiKey || apiKey === '') {
    logger.error('❌ RESEND_API_KEY manquante ou vide - Les emails ne fonctionneront pas');
    return false;
  }
  
  if (!fromEmail || fromEmail === 'onboarding@resend.dev') {
    logger.warn('⚠️ RESEND_FROM_EMAIL invalide - Utilisation du fallback');
  }
...
const getSenderEmail = (): string => {
  ...
  // En production, ne jamais utiliser onboarding@resend.dev
  const isProduction = 
...
export const sendTrackedEmail = async (
  ...
  try {
    ...
    logger.info('📨 Tentative d\'envoi d\'email avec tracking:', {
      from: senderEmail,
      to: recipientEmail,
      subject: subject
    });
    ...
    if (error instanceof Error) {
      if (error.message.includes('API')) {
        throw new Error('Configuration email invalide. Vérifiez RESEND_API_KEY dans les variables d\'environnement.');
      }
      if (error.message.includes('from')) {
        throw new Error('Email d\'envoi invalide. Configurez RESEND_FROM_EMAIL avec un domaine vérifié.');
      }
    }
...
export const testEmailConfiguration = async (): Promise<boolean> => {
  try {
    ...
    const senderEmail = getSenderEmail();
    logger.info(`✅ Configuration email Resend validée (from: ${senderEmail})`);
    return true;
  } catch (error) {
    ...
    const { data, error } = await resendClient.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <hr>
          <p><small>Envoyé depuis: ${senderEmail}</small></p>
        </div>
      `,
    });
...
export const sendWelcomeEmail = async (user: {
  ...
  try {
    ...
    if (error) {
      logger.error('❌ Erreur Resend pour email de bienvenue:', error);
